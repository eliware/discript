...
describe('permission command handler', () => {
  ...
  test.each([
    [{ allow: 'View,' }, ['set', 't', { allow: ['View'] }, { reason: undefined }]],
    [{ deny: 'Send,' }, ['set', 't', { deny: ['Send'] }, { reason: undefined }]],
  ])('accepts one permission direction at a time', (options, value) => expect(createPermissionsHandler({ command: ['permissions', 'set'], options: { channel: 'c', target: 't', ...options }, api })).toEqual({ handled: true, value }));
