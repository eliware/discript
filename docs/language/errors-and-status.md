...
Parser failures include a `PARSE_ERROR` code, exit status `3`, and, when available, `details.line`, `details.column`, and `details.offset`. Runtime errors remain structured but do not yet expose a full script call stack.
