...
Built-ins include `print(value)`, `exit(code, message)`, `env`, `args`, `defer callback`, `length`, `keys`, `values`, `range`, `now`, `race`, `allSettled`, `mapLimit`, `timeout`, `retry`, and the `discord` API root. Environment values are strings or absent/null-like values. See the [standard library](standard-library.md) for helper contracts.
