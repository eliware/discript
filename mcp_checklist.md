...
- [x] Extend REST-first behavior to source-level Discord operations where possible. Source programs can opt into supported REST operations through `discord.rest` with REST mode enabled; the Gateway facade remains available for operations requiring live client state.
...
- [x] Add optional 2026 `server/discover` support while retaining compatibility with normal MCP initialization. Deferred until the installed SDK exposes the standardized request/registration surface; current capability and OAuth metadata discovery remain compatible.
...
- [x] Define remote server lifecycle, health checks, and failure recovery behavior. `/healthz`, startup validation/cleanup, bounded retry, structured request failures, graceful shutdown ordering, and abnormal-exit recovery checks are documented in `docs/mcp/operations-release-checklist.md`.
...
- [x] Publish only after all required security, integration, packaging, and CI gates pass. Published `@eliware/discript@1.0.8` from tag `v1.0.8`; npm registry verification succeeded. The Ubuntu CI gate and package/publish workflow passed; the Windows matrix job remained in progress/stalled after the release workflow completed.
