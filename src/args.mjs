export function parseArgs(argv = []) {
  ...
  for (let index = 0; index < argv.length; index += 1) {
    ...
    if (token === '-h') options.help = true;
      ...
      else if (['eval', 'timeout', 'duration', 'start', 'reason', 'emoji', 'sticker', 'messages', 'invite', 'event', 'guild', 'channel', 'message', 'thread', 'user', 'role', 'content', 'name', 'description', 'location', 'event_type', 'output', 'file', 'tags'].includes(key)) options[key] = requireValue(argv, ++index, `--${rawKey}`);
