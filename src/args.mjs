export function parseArgs(argv = []) {
  ...
  for (let index = 0; index < argv.length; index += 1) {
    ...
    if (token === '-h') options.help = true;
      ...
      else if (['eval', 'timeout', 'duration', 'reason', 'guild', 'channel', 'message', 'thread', 'user', 'role', 'content', 'name', 'output'].includes(key)) options[key] = requireValue(argv, ++index, `--${rawKey}`);
