...
export async function evaluate(program, globals = {}, { scope: sharedScope, baseDir = process.cwd() } = {}) {
  const { baseScope, context: scopeContext, scope, createChildScope } = createScope(globals, sharedScope);
  ...
  evaluateExpression = createExpressionEvaluator({ scope, scopeContext, createChildScope, evaluateBlock, evaluateExpression: expression => evaluateExpression(expression), normalizeError, runtimeError, ScriptExit });
  evaluateStatement = createStatementEvaluator({ scope, scopeContext, createChildScope, evaluateBlock, evaluateExpression: expression => evaluateExpression(expression), runtimeError, ReturnSignal, createClosure, baseDir });
