...
export function parse(source) {
  ...
  const consume = (type, message) => stream.consume(type, message, syntaxError);
  let parseBlock;
  const expressions = createExpressionParser({ peek, match, take, consume, syntaxError, precedence, parseBlock: () => parseBlock() });
  const statements = createStatementParser({ peek, match, take, consume, syntaxError, parseExpression: expressions.parseExpression, parseBlock: () => parseBlock() });
  parseBlock = () => {
    consume('{', 'Expected `{` to begin a block.');
    const body = [];
    while (!atEnd() && !match('}')) { if (match(';')) continue; body.push(statements()); match(';'); }
    if (stream.previous()?.value !== '}') throw syntaxError('Expected `}` to close the block.');
    return body;
  };
  const body = [];
