...
| Topic | Examples |
| --- | --- |
| Language fundamentals | [conditionals](fundamentals/conditionals.ds), [loops](fundamentals/loops.ds), [while](fundamentals/while.ds), [callbacks](fundamentals/callbacks.ds), [parallel](fundamentals/parallel.ds), [reduce](fundamentals/reduce.ds), [block comments](fundamentals/block-comments.ds) |
| Agent output | [result envelopes](agents/result-envelope.ds), [branching](agents/branch-on-result.ds), [sanitized output](agents/sanitized-output.ds), [resource joins](agents/join-resource-data.ds), [JSONL monitoring](agent/jsonl-monitor.ds) |
...
| Discord lifecycle | [message CRUD](discord/message-crud.ds), [role lifecycle](discord/role-lifecycle.ds), [thread lifecycle](discord/thread-lifecycle.ds), [webhook lifecycle](discord/webhook-lifecycle.ds), [invite lifecycle](discord/invite-lifecycle.ds), [scheduled events](discord/scheduled-event-lifecycle.ds) |
...
| MCP and transports | [remote inventory](mcp/remote-inventory.ds), [discover before run](mcp/discover-before-run.ds), [dry-run plan](mcp/run-dry-plan.ds), [hybrid daemon](mcp/hybrid-daemon-workflow.ds), [remote errors](mcp/remote-error-handling.ds) |
